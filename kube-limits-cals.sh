...
for ns in $(kubectl get ns --no-headers=true | awk '{print $1}'); do
	for type in deploy ds statefulsets; do
		for deploy in $(kubectl -n "${ns}" get "${type}" --no-headers=true 2>/dev/null | awk '{print $1}' ) ; do
			...
			replicas=$(cat "${tmp}" | jq '" \(.spec.replicas) "' | sed 's/"//g' | sed -r 's/\ //g')
			for container in $(cat "${tmp}" | jq '" \(.spec.template.spec.containers[].name)"' | sed 's/"//g' | sed -r 's/\ //g'); do
	...
	awk '
		...
		{ 
			print $1,$2,$3,$4,$5,calc_cpu($6),calc_ram($7)
		}'
